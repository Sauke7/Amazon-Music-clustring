...
def winsorize_df(df_features: pd.DataFrame, lower_q=0.01, upper_q=0.99):
    """
    Winsorize numeric features by clipping each column to [lower_q, upper_q] quantiles.
    Returns a new DataFrame with the same index/columns.
    """
    df_w = df_features.copy()

    # compute per-column caps
    lower = df_w.quantile(lower_q)
    upper = df_w.quantile(upper_q)

    # clip each column to its quantile range
    df_w = df_w.clip(lower=lower, upper=upper, axis=1)

    # quick sanity check: how many values were clipped
    clipped_count = (df_w.ne(df_features)).sum().sum()
    total_values = df_features.shape[0] * df_features.shape[1]
    print(f"Winsorization: clipped {clipped_count} / {total_values} values "
          f"({(clipped_count/total_values)*100:.2f}%) using "
          f"{lower_q:.0%}-{upper_q:.0%} quantiles.")

    return df_w


# --- Apply winsorization BEFORE scaling ---
df_features = winsorize_df(df_features, lower_q=0.01, upper_q=0.99)
